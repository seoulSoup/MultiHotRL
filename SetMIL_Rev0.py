...
class TupleTransitionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        assert assign.ndim == 2 and assign.shape[1] == 44
        assert before.ndim == 2
        assert after.ndim == 2
        assert assign.shape[0] == before.shape[0]
        assert assign.shape[0] == after.shape[0]

        before = np.nan_to_num(before, nan=0.0, posinf=self.clip_value, neginf=-self.clip_value)
        after = np.nan_to_num(after, nan=0.0, posinf=self.clip_value, neginf=-self.clip_value)
...
class RowSetTransformerEncoder(nn.Module):
    """
    row 하나 = 순서 없는 measurement point set
...
# =========================================================
# 5. Row-wise Set-MIL Offline Q Model
# =========================================================
...
class SetMILOfflineQ(nn.Module):
    ...
    def forward_single(self, assign, matrix):
        ...
        cross_weights = cross_weights.squeeze(1)  # (N, 44)
    ...
    def select_action(self, assign, matrix):
        ...
        return out["q_values"].argmax().item(), out
...
def compute_rowwise_reward(
    ...
    """
    row-wise anomaly improvement 기반 reward.

    risk는 출력/해석용으로 유지하지만,
    reward는 초기 안정성을 위해 anomaly 개선량만 사용.
    """
...
def row_risk_aux_loss(row_risk, row_anomaly):
    """
    완전 supervised label은 아니고,
    row_risk가 anomaly 크기 순서를 대략 따라가도록 하는 보조 loss.
    """

    if row_anomaly.numel() <= 1:
        return row_risk.sum() * 0.0

    target = row_anomaly.detach()
    ...
    return F.mse_loss(row_risk, target)
...
def train_epoch(
    ...
    for batch in loader:
        ...
        for assign, before, after, action in batch:
            ...
            aux_loss = row_risk_aux_loss(
                row_risk=out["row_risk"],
                row_anomaly=out["row_anomaly"],
            )

            loss = q_loss + aux_coef * aux_loss
            loss = loss / bs
            loss.backward()

            batch_loss += loss.detach().item() * bs
    ...
    return {
        ...
        "aux_loss": total_aux / max(total_n, 1),
...
def evaluate(model, loader, device):
    ...
    for batch in loader:
        for assign, before, after, action in batch:
            ...
            total_improve += rinfo["improve_ratio"].item()
            total_fail += rinfo["dut_fail"].item()
            total_n += 1
...
def main():
    ...
    parser.add_argument("--save_path", type=str, default="set_mil_offline_q.pt")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"aux={log['aux_loss']:.6f} "
            f"reward={log['reward']:.6f} "
            f"improve={log['improve']:.4f} "
            f"dut_fail={log['dut_fail']:.4f} "
            f"| eval_reward={eval_log['reward']:.6f}"
        )
